...
export default function PropertyWorkspace() {
  ...
  const validate = (publicationStatus) => {
    ...
    if (!form.operationType) errors.push('Selecciona si es venta o alquiler.');
    if (!form.priceOnRequest && !(Number(form.price) > 0)) errors.push('Escribe un precio válido o activa “Precio a consultar”.');

    if (publicationStatus === 'published') {
      if (!form.description.trim()) errors.push('Agrega una descripción antes de publicar.');
      if (!form.department) errors.push('Selecciona el departamento antes de publicar.');
      if (!form.city.trim()) errors.push('Escribe la ciudad o municipio antes de publicar.');
      if (!form.coverImage) errors.push('Selecciona una fotografía de portada antes de publicar.');
    }

    if (errors.length) {
      setError(errors.join(' '));
      document.querySelector('.property-workspace__messages')?.scrollIntoView({ behavior: 'smooth', block: 'center' });
      return false;
    }
    return true;
  };
    ...
    <div className="property-workspace">
      <header className="property-workspace__header">
        <div>
          ...
          <p>Editor avanzado inspirado en el flujo operativo de Diamantes Realty Group.</p>
        </div>
      ...
      <div className="property-workspace__layout">
        <aside className="property-workspace__navigation" aria-label="Secciones del formulario">
          <strong>Secciones</strong>
          {propertySections.map(([key, label], index) => (
            <a key={key} href={`#property-${key}`}>
              <span>{index + 1}</span>
              {label}
            </a>
          ))}
        ...
        <form className="property-workspace__form" onSubmit={(event) => { event.preventDefault(); submit(form.publicationStatus); }}>
          <div className="property-workspace__messages" aria-live="polite">
            {message && <p className="success"><CheckCircle2 size={18} /> {message}</p>}
            {error && <p className="error"><AlertCircle size={18} /> {error}</p>}
          </div>
          ...
          <section id="property-location" className="property-workspace__section">
            ...
            <div className="property-workspace__grid">
              <SelectField
                ...
                required={form.publicationStatus === 'published'}
              />
              <Field label="Ciudad o municipio" value={form.city} onChange={(value) => set('city', value)} placeholder="Ej. Matagalpa" required={form.publicationStatus === 'published'} />
          ...
          <section id="property-marketing" className="property-workspace__section">
            <div className="property-workspace__section-heading">
              <span>06</span>
              <div>
                <h2>Presentación y posicionamiento</h2>
                <p>Controla las etiquetas comerciales y el texto usado por buscadores.</p>
              </div>
            </div>

            <ChoiceGrid
              title="Etiquetas destacadas"
              items={highlightTagOptions}
              selected={form.highlightTags}
              onToggle={(value) => toggleListValue('highlightTags', value, 2)}
              limit={2}
              help="Se muestran como insignias en las tarjetas. Máximo dos."
            />

            <div className="property-workspace__grid">
              <Field label="Título SEO" value={form.seoTitle} onChange={(value) => set('seoTitle', value.slice(0, 70))} placeholder={form.title || 'Título para Google'} help={`${form.seoTitle.length}/70 caracteres`} />
              <TextAreaField label="Descripción SEO" value={form.seoDescription} onChange={(value) => set('seoDescription', value.slice(0, 160))} placeholder="Descripción breve para buscadores y redes sociales." rows={3} help={`${form.seoDescription.length}/160 caracteres`} />
            </div>
          </section>

          <div className="property-workspace__sticky-actions">
            <div>
              <strong>{form.title || 'Propiedad sin título'}</strong>
              <span>{dirty ? 'Hay cambios pendientes' : 'Todos los cambios están guardados'}</span>
            </div>
            <div>
              ...
              <button className="btn primary" type="button" disabled={saving || uploading} onClick={() => submit('published')}>
